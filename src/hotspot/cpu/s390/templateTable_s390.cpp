...
void TemplateTable::if_acmp(Condition cc) {
  // TODO: if_acmp untested
  transition(atos, vtos);
  // Assume branch is more often taken than not (loops use backward branches).
  Label taken, not_taken;
  // Use Z_ARG5 instead of Z_ARG2 to avoid register conflicts in call_VM
...
void TemplateTable::invoke_is_substitutable(Register aobj, Register bobj, Label& is_subst, Label& not_subst) {
  // TODO: untested invoke_is_substitutable
...
void TemplateTable::getfield_or_static(int byte_no, bool is_static, RewriteControl rc) {
                      ...
                      // to here is compensated for by the fallthru to "Done".
  {
    ...
    if (!Arguments::is_valhalla_enabled()) {
      ...
      if (is_static) {
        // __ untested("getfield_or_static");
        __ load_heap_oop(Z_tos, field, oopLoad_tmp1, oopLoad_tmp2);
        __ push(atos);
      } else {
        // TODO: not yet stepped through
        Label is_flat;
        __ test_field_is_flat(flags, Z_R0_scratch, is_flat);
        __ load_heap_oop(Z_tos, field, oopLoad_tmp1, oopLoad_tmp2);
        __ push(atos);
        if (do_rewrite) {
          patch_bytecode(Bytecodes::_fast_agetfield, bc_reg, patch_tmp);
...
void TemplateTable::putfield_or_static(int byte_no, bool is_static, RewriteControl rc) {
  ...
  // TODO: someone free a fucking register
...
void TemplateTable::_new() {
  ...
  // TODO: __ untested("TemplateTable::_new");
